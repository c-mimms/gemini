# Task: Mimms Museum Content Generation (Museum Engine)
...
Save your output to a `.html` file in `museum_engine/articles/`.
Filename format: `YYYY-MM-DD_short-slug-name.html`
...
<div class="metadata" style="display:none;">
    ...
    <meta name="tag" content="[Format Name] | [Theme]">
</div>
```
...
### Execution Strategy (Parity)

1. **Check for Duplicates:** List contents of `museum_engine/articles/` and identify existing topics and content types. Do NOT research or generate content for a topic already covered.
2. **Pick the Format:** Semi-randomly decide which of the 9 formats you will generate today, balancing across recent runs.
3. **Brainstorm a NEW Topic:** Based on the chosen format, pick a new artifact or theme.
4. **Research:** Use web research to ensure technical and historical accuracy.
5. **Handle Images:** Download images with permissible licenses. Prefer Wikimedia Commons via `Special:FilePath`.
6. **Author the Content:** Produce HTML using the provided CSS tool inventory.
7. **Save to File:** Write directly to `museum_engine/articles/`.

### Publish the Site
Run:
```bash
python3 /Users/chris/code/gemini/static_site/build_museum.py \
  --source /Users/chris/code/gemini/museum_engine/articles/ \
