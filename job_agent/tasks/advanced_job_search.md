...
## Data & Log Files
- **Job Database**: A local file at `/Users/chris/code/gemini/job_agent/data/job_database.json` will be used to store structured data for every job that is processed. This prevents re-processing and allows for future analysis.
- **Log File**: A simple log at `/Users/chris/code/gemini/job_agent/data/seen_emails.log` will store the message IDs of emails that have already been processed, to avoid re-reading old emails.
...
2.  **Read New Emails**:
    ...
    -   Use `python /Users/chris/code/gemini/job_agent/bin/read_email.py` to check for recent, unread emails with subjects related to "New jobs for you in Edinburgh".
    -   For each email found, check its ID against `../../data/seen_emails.log`. If already processed, print "Skipping already processed email" and skip it.
...
4.  **Process Each Job URL**:
    -   For each new URL, check if it already exists in the `../../data/job_database.json`. If so, print "Skipping already processed job" and skip it.
...
8.  **Take Action**:
    ...
    -   **If a job IS a good fit**:
        -   Format and send the summary email using `python /Users/chris/code/gemini/job_agent/bin/send_email.py`.
